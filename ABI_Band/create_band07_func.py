...
def create_T07(path, filename):
    ...
    T_07 = T_07.expand_dims({'time':[dt]})
    
    return T_07

#--- Running the function to create the BTD xarray

T_07 = create_T07(path, filename)

#--- Creating the datetime string

dt = T_07.time[0].values.astype(datetime.datetime)
python_dt = datetime.datetime.utcfromtimestamp(dt * 1e-9)
dt_str = python_dt.strftime("%Y_%m_%d_%HH_%MM")

#--- Saving the file in /ABI_Band_07

file = open(result_path+"goes_e_band07_"+dt_str, 'wb')
T07_file = pickle.dump(T_07, file)
file.close()

print('Success --- Band 07 for '+dt_str+'  produced')
